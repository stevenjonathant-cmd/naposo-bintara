...
export default async function ChordDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <main className="section-shell overflow-hidden">
      <Link href="/chords" className="church-button-light">
        <ArrowLeft size={17} /> Back to list
      </Link>

      <section className="mt-6 min-w-0 max-w-full church-card p-5 sm:p-7">
        <p className="eyebrow text-ember">Buku Ende Chords</p>
        <h1 className="mt-3 text-4xl font-black uppercase leading-none text-ink sm:text-6xl">{songHeading(song)}</h1>
      ...
      <section className="mt-6 grid min-w-0 max-w-full gap-6">
        <div className="min-w-0 max-w-full church-card p-4">
          <ChordImageViewer images={song.image_urls} title={songHeading(song)} songId={song.id} />
        </div>

        <div className="grid min-w-0 max-w-full gap-5">
          <div className="min-w-0 max-w-full church-card p-5">
            <dl className="grid gap-3 text-sm sm:grid-cols-3">
              <div className="rounded-2xl border border-ink/10 bg-neutral-50 p-3">
