...
export function ServiceCard({ service }: { service: Service }) {
  return (
    <article className="glass-panel rounded p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h3 className="mt-2 text-xl font-black text-ink">{service.title}</h3>
          {service.theme ? <p className="mt-1 font-semibold text-graphite/60">{service.theme}</p> : null}
        </div>
        <span className="grid h-11 w-11 shrink-0 place-items-center rounded bg-ink text-white">
          <Music2 size={20} />
        </span>
      </div>
      ...
      <dl className="mt-5 grid gap-3">
        {service.assignments.map((assignment) => (
          <div key={assignment.role} className="grid gap-1 rounded border border-ink/10 bg-white/70 px-3 py-2 sm:grid-cols-[150px_1fr]">
