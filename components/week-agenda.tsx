  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.24em] text-teal">Public agenda</p>
          <h2 className="mt-2 text-3xl font-black text-ink sm:text-4xl">Agenda This Week</h2>
        </div>
        <p className="max-w-xl text-sm font-semibold leading-6 text-graphite/70">
      ...
      <div className="grid gap-3 lg:grid-cols-4">
        {items.map((item) => (
          <article key={item.id} className="glass-panel rounded p-4">
            <div className={`flex items-center justify-between gap-3 rounded border px-3 py-2 ${accentStyles[item.accent]}`}>
